...
// the other as a CCTK_REAL4 grid function (state4_evol/rhs4_evol). This
// exercises ODESolvers::statecomp_t's per-group mixed-precision support
...
// converged, but not bitwise-exact, numerical integration:
// double ~1e-12 (par/testreal4_evol.par uses a small enough
...
extern "C" void TestReal4_Evol_Initial(CCTK_ARGUMENTS) {
  ...
  grid.loop_all_device<1, 1, 1>(
      ...
      [=] CCTK_DEVICE(const Loop::PointDesc &p) CCTK_ATTRIBUTE_ALWAYS_INLINE {
        ...
        // validity check at the very first CycleTimelevels. Zero-init
        // them here too; their value is never asserted on outside the
        // evol test.
...
extern "C" void TestReal4_Evol_Check(CCTK_ARGUMENTS) {
  ...
  CCTK_VINFO("TestReal4-evol: PASS (%d points checked, t=%.6g, "
             "state8=%.15g, state4=%.7g)",
             n_checked, double(cctk_time), double(exact8), double(exact4));
}
